...
namespace gnsstk
{
   ...
   NEW_EXCEPTION_CLASS(VectorException, gnsstk::Exception);
      ...
      /**
       * A base class for a vector that does not allow modification of
       * the internal vector.  BaseClass is the base class that
   ...
   public:
      ...
      /**
       * A vector base class that allows modification of the internal
       * representation.
       */
   template <class T, class BaseClass>
...
#define VecBaseNewAssignOperator(funcName, op)                          \
   ...
   public:
      ...
      { return static_cast<const BaseClass*>(this)->start(); }
         /** How many elements separate the i'th element from the
          * i+1'th element. */
      size_t stride() const
