...
namespace pod
{
    ...
    {
        for (auto& it : ifb2ss)
            stochasticModels.insert(std::make_pair(it.first, std::make_unique<StochasticModel>()));
    }
    ...
    void InterFrequencyBiases::updateH(const gpstk::gnssRinex& gData, const gpstk::TypeIDSet& obsTypes, gpstk::Matrix<double>& H, int& col_0)
    {
