...
covariance matrix.");
   ...
   void SolverPPP::updateCurPar(gnssRinex& gRin)
   {
       ...
       if (numGLN < 2)
       {
           ...
           //defaultEqDef.body.erase(TypeID::recCdtGLO);
       }
       else
       {
   ...
   void SolverPPP::updateMatrices(Matrix<double> & phiMatrix, Matrix<double> & qMatrix, gnssRinex& gData)
   {
       ...
       if (defaultEqDef.body.find(TypeID::recCdtGLO) != defaultEqDef.body.end())
       {
           ...
           phiMatrix(5, 5) = pInterSysBiasStoModel->getPhi();
           qMatrix(5, 5) = pInterSysBiasStoModel->getQ();
       }
   ...
   SolverPPP& SolverPPP::setNEU( bool useNEU )
   {
      ...
      tempSet.insert(TypeID::recCdtGLO);  // #6
