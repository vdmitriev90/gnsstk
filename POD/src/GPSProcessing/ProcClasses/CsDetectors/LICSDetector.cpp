...
namespace pod
{
    ...
    /* Common constructor
       ...
       */
    LICSDetector::LICSDetector(const double& mThr, const double& drift, const double& dtMax, const bool& use)
        : useLLI(use)
    {
        setDeltaTMax(dtMax);
    ...
    /* Returns a satTypeValueMap object, adding the new data generated
       ...
       */
    SatTypePtrMap& LICSDetector::Process(const CommonTime& epoch, SatTypePtrMap& gData, const short& epochflag)
    {
        ...
        try
        {
            ...
            for (auto it = gData.begin(); it != gData.end(); ++it)
            {
                ...
                (*it->second)[csFlag1] +=
                    getDetection(epoch, (*it).first, (*it->second), epochflag, value1, lli1, lli2);
                                      ...
                                      const double& lli2)
    {
        ...
        // has elapsed
        // Note: If tvMap LLI indexes don't exist, then 0
        // will be returned and those tests will pass
        const auto first_lli = resolver_.firstLLI(sat.system);
        if ((tvMap(first_lli) == 1.0) || (tvMap(first_lli) == 3.0)
            || (tvMap(first_lli) == 5.0) || (tvMap(first_lli) == 7.0))
        {
            tempLLI1 = 1.0;
        }

        const auto second_lli = resolver_.secondLLI(sat.system);
        if ((tvMap(second_lli) == 1.0) || (tvMap(second_lli) == 3.0)
            || (tvMap(second_lli) == 5.0) || (tvMap(second_lli) == 7.0))
        {
            tempLLI2 = 1.0;
        }

        if ((epochflag == 1) || (epochflag == 6) || (tempLLI1 == 1.0) || (tempLLI2 == 1.0)
