...
namespace pod
{
    ...
    const double LIGTH_MS = 299792.458;
    // Rtcm3MessageBase::Initializer Rtcm3MessageBase::initializer;
    std::set<rtcm3_msg_uptr> Rtcm3MessageBase::init_msg_set()
    {
        std::set<rtcm3_msg_uptr> msgs;
        msgs.emplace(std::make_unique<Rtcm3_1008>());
        return msgs;
    }
    std::map<int, rtcm3_msg_uptr> Rtcm3MessageBase::init_msg_map()
    {
    ...
    bool Rtcm3_1008::parse(const BitReader& buffer, Rtcm3Decoder& decoder)
    {
        int ref_id = buffer.u32(12, 12);
        int n = buffer.u32(24, 8);
        auto ant_discr = buffer.str(32, n * 8);
        std::cout << ref_id << " " << ant_discr << std::endl;
        return true;
    }

    bool Rtcm3_GpsObs::parseHeader(const BitReader& buffer, Rtcm3Decoder& decoder)
    {
        int ref_id = buffer.u32(12, 12);
        int tow = buffer.u32(24, 30);
        bool syncFlag = buffer.bit(54);
        nSats = buffer.u32(55, 5);
        std::cout << "SyncFlag: " << syncFlag << " tow: " << tow << " ngps: " << nSats << std::endl;
        return true;
    }

    bool Rtcm3_1004::parse(const BitReader& buffer, Rtcm3Decoder& decoder)
    {
        if (parseHeader(buffer, decoder))
            for (size_t i = 0; i < nSats; i++)
            {
                if (!parseSatData(BitReader(buffer, 64 + 125 * i, 125), decoder))
                    return false;
            }
    ...
    bool Rtcm3_1004::parseSatData(const BitReader& buffer, Rtcm3Decoder& decoder)
    {
        int id = buffer.u32(0, 6);
        SatID satId(id, SatelliteSystem::GPS);
        TypeID typeIdL1 = buffer.bit(6) ? TypeID::P1 : TypeID::C1;

        int prL1 = buffer.u32(7, 24);
        int dL1phase = buffer.i32(31, 20);
        int L1CodeAmb = buffer.u32(58, 8);
        int cnrL1 = buffer.u32(66, 8);

        int typeL2code = buffer.u32(74, 2);
        TypeID typeIdL2 = typeL2code > 0 ? TypeID::P2 : TypeID::C2;

        int dL2code = buffer.i32(76, 14);
        int dL2phase = buffer.i32(90, 20);
        int cnrL2 = buffer.u32(117, 8);
