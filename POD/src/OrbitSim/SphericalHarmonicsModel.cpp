...
namespace pod
{
    ...
    void SphericalHarmonicsModel::computeVW(Vector<double> r_bf)
    {
        ...
        for (int m = 1; m <= (gmData_.desiredOrder + 2); m++)
        {
            ...
            {
                V[n][m] =
                    ((2 * n - 1) * z0 * V[n - 1][m] - (n + m - 1) * rho * V[n - 2][m]) / (n - m);
                W[n][m] =
                    ((2 * n - 1) * z0 * W[n - 1][m] - (n + m - 1) * rho * W[n - 2][m]) / (n - m);
            }
        ...
        for (int m = 0; m <= gmData_.desiredOrder; m++)
        {
            for (int n = m; n <= gmData_.desiredDegree; n++)
            {
                ...
                if (m > 0)
                {
                    ...
                    xz += f1 * (C * V[n + 2][m + 1] + S * W[n + 2][m + 1])
                          - f2 * (C * V[n + 2][m - 1] + S * W[n + 2][m - 1]);
                    yz +=
                        f1 * (C * W[n + 2][m + 1] - S * V[n + 2][m + 1])
                        + f2 * (C * W[n + 2][m - 1] - S * V[n + 2][m - 1]); //* bug in JAT, I fix it
                    ...
                    if (m == 1)
                    {
                        ...
                        xx += 0.25
                              * (C * V[n + 2][3] + S * W[n + 2][3]
                                 - Fac * (3.0 * C * V[n + 2][1] + S * W[n + 2][1]));
                        xy += 0.25
                              * (C * W[n + 2][3] - S * V[n + 2][3]
                                 - Fac * (C * W[n + 2][1] + S * V[n + 2][1]));
                    }
                    if (m > 1)
                    {
                        f1 = 2.0 * (n - m + 2) * (n - m + 1);
                        ...
                        xx += 0.25
                              * (C * V[n + 2][m + 2] + S * W[n + 2][m + 2]
                                 - f1 * (C * V[n + 2][m] + S * W[n + 2][m])
                                 + f2 * (C * V[n + 2][m - 2] + S * W[n + 2][m - 2]));

                        xy += 0.25
                              * (C * W[n + 2][m + 2] - S * V[n + 2][m + 2]
                                 + f2 * (-C * W[n + 2][m - 2] + S * V[n + 2][m - 2]));
                    }
                }
            }
    ...
    void SphericalHarmonicsModel::correctCSTides(Epoch t,
                                                 bool solidFlag,
                                                 bool oceanFlag,
                                                 bool poleFlag)
    {
