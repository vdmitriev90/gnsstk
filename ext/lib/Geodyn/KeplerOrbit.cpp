...
namespace gnsstk
{
   ...
   double KeplerOrbit::EccentricAnomaly(double M, double e)
   {
      ...
      M = Modulo(M, 2.0*gnsstk::PI);   
      if (e<0.8) E=M; else E= gnsstk::PI;
   ...
   Vector<double> KeplerOrbit::State( double GM, const Vector<double>& Kep, double dt )
   {
      ...
      const Matrix<double> PQW = rotation(-Omega, 3) * rotation(-i, 1) * rotation(-omega, 3);
   ...
   Vector<double> KeplerOrbit::Elements( double GM, const Vector<double>& y )
   {
      ...
      Omega = Modulo(Omega,2.0*gnsstk::PI);
      ...
      double M  = Modulo(E-eSinE,2.0* gnsstk::PI);                // Mean anomaly

      double nu = std::atan2(std::sqrt(1.0-e2)*eSinE, eCosE-e2);     // True anomaly

      double omega = Modulo(u-nu,2.0*gnsstk::PI);                // Arg. of perihelion 
      ...
      const Vector<double>& ra, const Vector<double>& rb )
   {
      ...
      omega = Modulo(u-nu,2.0*gnsstk::PI);
      ...
      if (e<1.0) {
         ...
         M = Modulo( E - e*std::sin(E), 2.0*gnsstk::PI );
      }
      else 
      {
      ...
      gnsstk::Vector<double> Kep(6,0.0);
      Kep(0) = a;
      Kep(1) = e;
      Kep(2) = i;
      Kep(3) = Omega;
      ...
      Vector<double>& Y, Matrix<double>& dYdY0 )
   {
      ...
      gnsstk::Vector<double>  A0(6);
      gnsstk::Matrix<double>  dY0dA0(6,6), dYdA0(6,6), dA0dY0(6,6);
   ...
   void KeplerOrbit::test()
   {
      ...
      const double Deg = 180.0 / gnsstk::PI;
